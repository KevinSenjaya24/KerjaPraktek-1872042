...
@section('pageTitle', 'CATEGORY RESERVATION MANAGEMENT')

@section('actionBar')
    <a href="{{ route('admin.categoryReservation.detail','add') }}" class="btn btn-outline btn-info pull-right m-l-20 waves-effect waves-light">Add</a>
@endSection
    ...
    <div class="panel panel-info">
        <div class="panel-wrapper collapse in" aria-expanded="true">
            <div class="panel-body">
                <h3 class="box-title m-b-0 black">Table Category Reservation</h3>
                <hr>
                    ...
                    <table id="myTable" class="table table-striped">
                        ...
                        @foreach($categories as $category)
                            <tr>
                                ...
                                <td>{{$category->kursi}}</td>
                                <td>
                                    <form method="POST" action="{{route('admin.categoryReservation.update')}}">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$category->id}}">
                                        <input type="hidden" name="name" value="{{$category->name}}">
                                        <input type="hidden" name="tanggal" value="{{$category->tanggal}}">
                                        <input type="hidden" name="kursi" value="{{$category->kursi}}">
                                        <input <?php if($category->active == 1){echo "checked";} ?> type="checkbox" name="active">
                                        <button type="submit" class="btn btn-success btn-approval">
                                            <i class="mdi mdi-check"></i>
                                        </button>
                                    </form>
                                </td>
                                <td><img src="{{asset('storage/'.$category->photo)}}" height="50"/></td>
                                <td>
                                <div class="row">
                                    <div class="col-md-2">
                                        <a href="{{ route('admin.categoryReservation.show',$category->id) }}" data-toggle="tooltip" title="Show" name="show" value="{{$category->id}}">
                                    ...
                                    <div class="col-md-2">
                                        <a href="{{ route('admin.categoryReservation.detail',$category->id) }}" name="edit_category" value="{{$category->id}}">
                                    ...
                                    <div class="col-md-2">
                                        <form method="POST" action="{{route('admin.categoryReservation.delete', $category->id)}}">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <!-- <a href="{{ route('admin.categoryReservation.generatePDF', $category->id) }}" class="btn btn-primary btn-reservation">PDF</a> -->
                                </div>
                                </td>
                            </tr>
                        @endforeach
