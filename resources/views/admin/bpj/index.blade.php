...
@section('pageTitle', 'BPJ Management')

@section('actionBar')
    <a href="{{ route('admin.bpj.detail','add') }}" class="btn btn-outline btn-info pull-right m-l-20 waves-effect waves-light">Add</a>
@endSection
    ...
    <div class="panel panel-info">
        <div class="panel-wrapper collapse in" aria-expanded="true">
            <div class="panel-body">
                <h3 class="box-title m-b-0 black">Table BPJ</h3>
                <hr>
                <div class="table-responsive">
                    <table id="myTable" class="table table-striped">
                        ...
                        <tbody>
                        <?php $no = 0;?>
                        @foreach($bpjemaat as $bpj)
                        <?php $no++ ;?>
                            <tr>
                                <td>{{ $no }}</td>
                                <td>{{$bpj->jemaat->nama}}</td>
                                <td>{{$bpj->divisi}}</td>
                                <td>{{$bpj->periode}}</td>
                                <td>
                                    <a href="{{ route('admin.bpj.detail',$bpj->id) }}" name="edit_id" value="{{$bpj->id}}">
                                        <button class="btn btn-primary">
                                            <i class="mdi mdi-border-color"></i>
                                        </button>
                                    </a>
                                    <form method="POST" action="{{route('admin.bpj.delete', $bpj->id)}}">
                                        @csrf
                                        @method('DELETE')
