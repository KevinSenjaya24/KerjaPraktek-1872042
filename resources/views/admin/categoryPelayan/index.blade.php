...
@section('pageTitle', 'CATEGORY PELAYAN MANAGEMENT')

@section('actionBar')
    <a href="{{ route('admin.categoryPelayan.detail','add') }}" class="btn btn-outline btn-info pull-right m-l-20 waves-effect waves-light">Add</a>
@endSection
    ...
    <div class="panel panel-info">
        <div class="panel-wrapper collapse in" aria-expanded="true">
            <div class="panel-body">
                <h3 class="box-title m-b-0 black">Table Category Pelayan</h3>
                <hr>
                <div class="table-responsive">
                    <table id="myTable" class="table table-striped">
                        ...
                        @foreach($pelayans as $pelayan)
                            <tr>
                                <td>{{$pelayan->id}}</td>
                                <td>{{$pelayan->category_name}}</td>
                                <td>{{$pelayan->ibadah}}</td>
                                <td>
                                    <a href="{{ route('admin.categoryPelayan.detail',$pelayan->id) }}" name="edit_category" value="{{$pelayan->id}}">
                                        <button class="btn btn-primary">
                                            <i class="mdi mdi-border-color"></i>
                                        </button>
                                    </a>
                                    <form method="POST" action="{{route('admin.categoryPelayan.delete', $pelayan->id)}}">
                                        @csrf
                                        @method('DELETE')
