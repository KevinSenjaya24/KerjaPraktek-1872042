...
@section('pageTitle', 'PELAYAN Management')

@section('actionBar')
    <a href="{{ route('admin.pelayan.detail','add') }}" class="btn btn-outline btn-info pull-right m-l-20 waves-effect waves-light">Add</a>
@endSection
    ...
    <div class="panel panel-info">
        <div class="panel-wrapper collapse in" aria-expanded="true">
            <div class="panel-body">
                <h3 class="box-title m-b-0 black">Table Pelayan</h3>
                <hr>
                <div class="table-responsive">
                    <table id="myTable" class="table table-striped">
                        ...
                        @foreach($pelayans as $pelayan)
                            <tr>
                                <td>{{$pelayan->id}}</td>
                                <td>{{$pelayan->jemaat->nama}}</td>
                                <td>{{$pelayan->CategoryPelayan->category_name}}</td>
                                <td>{{$pelayan->CategoryPelayan->ibadah}}</td>
                                <td>
                                    <a href="{{ route('admin.pelayan.detail',$pelayan->id) }}" name="edit_id" value="{{$pelayan->id}}">
                                        <button class="btn btn-primary">
                                            <i class="mdi mdi-border-color"></i>
                                        </button>
                                    </a>
                                    <form method="POST" action="{{route('admin.pelayan.delete', $pelayan->id)}}">
                                        @csrf
                                        @method('DELETE')
