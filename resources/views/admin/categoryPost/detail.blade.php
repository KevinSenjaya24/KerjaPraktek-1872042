...
@section('pageTitle', 'Category Post')
...
<div class="col-sm-8">
    <div class="white-box">
        <h3 class="box-title m-b-0 black">FORM CATEGORY POST DETAILS</h3>
        <hr>
        <form action="{{ route('admin.categoryPost.update') }}" method="post" enctype="multipart/form-data">
            @csrf
            <input type="hidden"  name="id" value="{{ (isset($category->id)?$category->id:"") }}"/>
            
            <div class="form-group">
                <label for="category_name">Category Name</label>
                <input type="text" name="category_name" class="form-control" id="category_name" value="{{ (isset($category->category_name) ? $category->category_name : "") }}" aria-describedby="category" placeholder="Enter Category Name">
            </div>
